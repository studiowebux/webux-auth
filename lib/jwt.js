...
const GenerateToken = (options, user, type, ip = null) => {
  return new Promise(async (resolve, reject) => {
    try {
      ...
      if (!saved) {
        return reject(type + " token not saved in Redis.");
      }
    ...
    try {
      client.hgetall(accessToken, (err, obj) => {
        if (err || !obj) {
          ...
          return reject("The information does not match.");
        }
      });
    } catch (e) {
      throw e;
        ...
        if (err) {
          ...
          client.hgetall(refreshToken, async (err, obj) => {
            ...
            if (obj.userID == userID && obj.type === "refresh") {
              const token = await GenerateToken(options, decoded, "access", ip);
              if (!token) {
